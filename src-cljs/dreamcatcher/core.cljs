...
; file:/home/robi/Clojure/dreamcatcher/src/dreamcatcher/core.clj
...
(defn position [array element]
  (let [a (clj->js array) b (clj->js element)]
    (.indexOf a b)))

;;(defn position [array element]
;;  (.indexOf (clj->js array) (clj->js element)))
  ...
  ([x state]
   (if-let [fix-choices (-> (if (map? x) x @x) :life state)]
     ...
     (let [direct-transitions (-> (get-transitions (get-stm x) state) keys vec)
           available? (fn [x y] (if (= -1 (position x y)) false true))
...
(defn next-choice
  ([instance] (next-choice instance :clockwise))
  ([instance m-character]
   (do
     (assert (:alive? instance) "Instance is not alive! First give it life...")
     (let [available-choices (get-choices instance)]
       (case m-character
         :clockwise (available-choices (-> (position available-choices (or (:last-choice instance) (:last-state instance)))
                                           inc
...
(defn act! 
  ...
  ([x m-character]
   (if-let [instance (if (map? x) x @x)]
     (do
       (assert (:alive? instance) "Instance is not alive! First give it life...")
       (move-to-next-choice x (next-choice instance)))
     (assert false "This is not STM instance"))))
