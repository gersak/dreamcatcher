  ...
  #?(:clj
      (:require [dreamcatcher.async :refer :all]
                [dreamcatcher.core :as d 
                 :refer [safe defstm with-stm move reach-state
                         state-changed? make-machine-instance 
                         data update-data! state make-machine-instance]]
                [clojure.core.async :as async 
                 :refer [mult mix chan tap 
                         admix close! put! take! go]])
     :cljs
     (:require [dreamcatcher.async :refer :all]
               [dreamcatcher.core :as d
                :refer [move state-changed? make-machine-instance  reach-state
                        update-data! data state make-machine-instance]]
...
(comment 
  (inject test-instance 1 {:name kifla
                           :counter 0})
